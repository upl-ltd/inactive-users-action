...
async function run() {
  ...
  const ssoClient = require('sso-client-library');
  for (const activity of userActivity) {
    const githubUsername = activity.jsonPayload.actor.login;

  try {
      const ssoUserDetails = await ssoClient.getUserDetails(githubUsername);
      const ssoUserEmail = ssoUserDetails.email;
    activity.jsonPayload.email = ssoUserEmail;
    } catch (error) {
      console.error(`Failed to fetch SSO details for ${githubUsername}: ${error.message}`);
    }
  }
